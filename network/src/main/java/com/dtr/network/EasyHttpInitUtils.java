...
public class EasyHttpInitUtils {
    public static void init(Application myApplication) {
        EasyHttp.init(myApplication);//默认初始化,必须调用

        //全局设置请求头
        HttpHeaders headers = new HttpHeaders();
//        headers.put("User-Agent", SystemInfoUtils.getUserAgent(myApplication, HttpConstants.APPID));
        //全局设置请求参数
        HttpParams params = new HttpParams();
        params.put("appId", HttpConstants.APPID);

        //以下设置的所有参数是全局参数,同样的参数可以在请求的时候再设置一遍,那么对于该请求来讲,请求中的参数会覆盖全局参数
        EasyHttp.getInstance()
                //可以全局统一设置全局URL
                .setBaseUrl(HttpConstants.Url)//设置全局URL  url只能是域名 或者域名+端口号
